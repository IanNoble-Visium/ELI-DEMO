...
try { ({ PredictionServiceClient } = require('@google-cloud/aiplatform')); } catch (_) { PredictionServiceClient = null; }

const PROJECT_ID = process.env.GOOGLE_PROJECT_ID;
const LOCATION = process.env.GOOGLE_LOCATION || 'us-central1';

function isConfigured() { return Boolean(PROJECT_ID && LOCATION && PredictionServiceClient); }

let predictionClient;
function getClient() {
  if (!predictionClient && PredictionServiceClient) {
    const clientOptions = { apiEndpoint: `${LOCATION}-aiplatform.googleapis.com` };
    const saJson = process.env.GOOGLE_SERVICE_ACCOUNT_JSON;
    if (saJson) {
      try { const credentials = JSON.parse(saJson); predictionClient = new PredictionServiceClient({ ...clientOptions, credentials }); }
      catch (_) { predictionClient = new PredictionServiceClient(clientOptions); }
    } else {
...
async function fetchImageAsBase64(url) {
  ...
  if (!resp.ok) throw new Error(`[vision] Failed to fetch image: ${resp.status} ${resp.statusText}`);
...
function normalizeBbox(b) {
  ...
  let yMin, xMin, yMax, xMax; const [a,b2,c,d] = b;
  if (a <= c && b2 <= d) { yMin=a; xMin=b2; yMax=c; xMax=d; } else { xMin=a; yMin=b2; xMax=c; yMax=d; }
  const w = Math.max(0, xMax - xMin); const h = Math.max(0, yMax - yMin);
...
async function predictObjectDetection({ imageBase64, confidenceThreshold = 0.3, maxPredictions = 50 }) {
  ...
  const instances = [{ content: imageBase64 }]; const parameters = { confidenceThreshold, maxPredictions };
  ...
  for (const p of preds) {
    ...
    for (let i = 0; i < Math.min(names.length, scores.length, boxes.length); i++) {
      const label = names[i]; const score = Number(scores[i]); const bbox = normalizeBbox(boxes[i]);
...
async function predictClassification({ imageBase64, maxPredictions = 5 }) {
  ...
  const instances = [{ content: imageBase64 }]; const parameters = { maxPredictions };
  const [response] = await client.predict({ endpoint, instances, parameters });
  const preds = (response?.predictions || []).map(p => JSON.parse(JSON.stringify(p)));
  const detections = [];
  for (const p of preds) {
    ...
    for (let i = 0; i < Math.min(names.length, scores.length); i++) {
      const label = names[i]; const score = Number(scores[i]);
...
async function runImageModel(imageUrl, opts = {}) {
  ...
  try {
    ...
    console.error('[vision] runImageModel error', e); return [];
  }
}

module.exports = { runImageModel, isVisionConfigured: isConfigured };
